...
def autoprose_sample(preds, temperature, interactive=False, top_n=3):
    '''
...
def autoprose_generate(model, vocab,
    ...
    while not end and len(text) < max_gen_length:
        ...
        if not interactive:
            ...
            options_index = autoprose_sample(
                model.predict(encoded_text, batch_size=1)[0],
                next_temperature,
...
def autoprose_encode_sequence(text, vocab, maxlen):
    '''
...
def autoprose_texts_from_file(file_path, header=True,
                               delim='\n', is_csv=False):
    '''
...
def autoprose_texts_from_file_context(file_path, header=True):
    '''
...
def autoprose_encode_cat(chars, vocab):
    '''
...
def synthesize(textgens, n=1, return_as_list=False, prefix='',
    ...
    for _ in iterable:
        ...
        while not end:
            ...
            gen_text, end = autoprose_generate(textgen.model,
                                                textgen.vocab,
                                                textgen.indices_char,
                                                temperature,
                                                textgen.parameters['max_length'],
                                                textgen.mtoken,
                                                textgen.parameters['word_level'],
                                                textgen.parameters.get(
                                                    'single_text', False),
                                                max_gen_length,
                                                prefix=gen_text,
                                                synthesize=True,
...
class generate_after_epoch(Callback):
    def __init__(self, autoprose, gen_epochs, max_gen_length):
        self.autoprose = autoprose
        self.gen_epochs = gen_epochs
        self.max_gen_length = max_gen_length

    def on_epoch_end(self, epoch, logs={}):
        if self.gen_epochs > 0 and (epoch+1) % self.gen_epochs == 0:
            self.autoprose.generate_samples(
                max_gen_length=self.max_gen_length)


class save_model_weights(Callback):
    def __init__(self, autoprose, num_epochs, save_epochs):
        self.autoprose = autoprose
        self.weights_name = autoprose.parameters['name']
        self.num_epochs = num_epochs
        self.save_epochs = save_epochs

    def on_epoch_end(self, epoch, logs={}):
        if len(self.autoprose.model.inputs) > 1:
            self.autoprose.model = Model(inputs=self.model.input[0],
        ...
        if self.save_epochs > 0 and (epoch+1) % self.save_epochs == 0 and self.num_epochs != (epoch+1):
            ...
            self.autoprose.model.save_weights(
